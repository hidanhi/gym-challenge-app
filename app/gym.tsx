...
export default function GymScreen() {
  const [ziel, setZiel] = useState(8000);
  ...
  useEffect(() => {
    (async () => {
      const savedDanny = await AsyncStorage.getItem('dannyCount');
      const savedNico = await AsyncStorage.getItem('nicoCount');
      const savedZiel = await AsyncStorage.getItem('ziel');
  ...
  return (
    <ScrollView contentContainerStyle={styles.container}>
      <Text style={styles.title}>🏋️‍♂️ Gym Challenge</Text>
      <Text style={styles.subtitle}>Jahresziel Liegestütze:</Text>
      <TextInput
